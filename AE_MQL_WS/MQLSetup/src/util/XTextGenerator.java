...
public class XTextGenerator {
	...
	*/
	...
	public static void writeXTextFile(){
		...
		//Query
		code += writeFindQueryCollection();
		code += writeFindQueryPattern();
		
		//SelectClause
		code += writeSelectClause();
	...
	private static String writeFindQueryCollection(){
		String code = 	"FindQueryCollection returns FindQueryCollection:"
					+	"\n		{FindQueryCollection}"
	...
	private static String writeFindQueryPattern(){
		
		String code = 	"FindQuery returns FindQuery:"
					+	"\n		{FindQuery}"
	...
	private static String writeSelectClause(){
		String code =	"SelectClause returns SelectClause:"
					+	"\n		{SelectClause}"
	...
	private static String writeConcreteRootClassReferences(EClass rc){
		...
		HashSet<EReference> referenceAndAttributeERefs = new HashSet<EReference>();
		for(EReference eRef: eReferences){
			if(eRef.getUpperBound()==1 && (eRef.getEType().getName().endsWith("AttrEClass") || eRef.getEType().getName().endsWith("_OrGroup")))
				referenceAndAttributeERefs.add(eRef);
		}
		
		Iterator<EReference> iter = referenceAndAttributeERefs.iterator();
		if(iter.hasNext()) code += "(";
		while(iter.hasNext()){
			EReference eRef = iter.next();
			String postfix = eRef.getEType().getName().endsWith("AttrEClass") ? "" : "_OrGroup";
	...
	private static String writeConcreteRootClassReferences_multipleCardinality(EClass rc){
		...
		for(EReference eRef: eReferences){
			if((eRef.getUpperBound()>1 || eRef.getUpperBound()==-1) && (eRef.getEType().getName().endsWith("AttrEClass") || eRef.getEType().getName().endsWith("_OrGroup")))
	...
	private static String writeConcreteOrGroups(){
		String code = "";
		...
		while(iter_rc.hasNext()){
			...
			for(EReference eRef: eRefs){
				if(eRef.getEType().getName().endsWith("OrGroup"))
			...
			while(iter_eRef.hasNext()){
				...
				if(eRef.getEType().getName().endsWith("_Containment_OrGroup")){
					code 	+=	patternName + " returns " + typeName + ":"
							+	"\n		{" + typeName + "}"
							+	"\n		'" + eRefName + "'((negated?='!')? & (transitive?='+')? & (generalized?='*')?) name=ID (':' containment+=" + rcTypeName  
							+	"\n('OR' containment+=" + rcTypeName + ")*)? ';'" 
							+	"\n;\n\n";
				}else if(eRef.getEType().getName().endsWith("_Reference_OrGroup")){
					code 	+=	patternName + " returns " + typeName + ":"
							+	"\n		{" + typeName + "}"
							+	"\n		'" + eRefName + "'((negated?='!')? & (transitive?='+')? & (generalized?='*')?) name=ID (':' reference+=[" + rcName + "|ID]" 
							+	"\n('OR' reference+=[" + rcName + "|ID])*)?';'" 
							+	"\n;\n\n";
				}
			}
	...
	private static String writeConcreteAttributes(){
		...
		for(EClass ac: QMMMetaModelBuilder_Collections.concreteAttributeEClasses.keySet()){
			...
			String generalized = "";
			if(!isEnumerationAttributeEClass(ac)) generalized = " & (generalized?='*')?";
			
			code 	+= 	ac.getName() + " returns " + ac.getName() + ":"
					+	"\n		{" + ac.getName() + "}"
					+	"\n		'" + ea.getName() + "' ((negated?='!')?" +  generalized + ") name=ID ";
	...
	private static boolean isEnumerationAttributeEClass(EClass ec){
		EList<EClass> superTypes = ec.getEAllSuperTypes();
		for(EClass st : superTypes) if(st.getName().equals("EnumerationAttributeEClass")) return true;
		return false;
	}
	...
	private static String writeConcreteEvaluationOperator(EClass op){
		...
		String code	=	op.getName() + " returns " + op.getName() + ":"
				+	"\n		{" + op.getName() + "}"
				+	"\n		'" + op.getEAnnotation("information").getDetails().get("operatorXTextLiteral" /*literal*/) + "('" + paramERef.getName() + "=" + paramERef.getEType().getName() + "')'(operator=" + op.getEStructuralFeature("operator").getEType().getName() + ")?"
				+	"\n;\n\n";
	...
	private static String writeConcreteFunctionalOperators(){
		...
		for(Collection<EClass> operators: QMMMetaModelBuilder_Collections.concreteFunctionalOperators.values()){
			for(EClass operator: operators){
				String name =  operator.getEAnnotation("information").getDetails().get("name");
				String negated = operator.getEStructuralFeature("negated") != null ? "(negated?='!')?" : "";
				if(!name.startsWith(".")) name = "." + operator.getEAnnotation("information").getDetails().get("name");
				code	+= 	operator.getName() + " returns " + operator.getName() + ":"
						+	"\n		{" + operator.getName() + "}"
						+	"\n		'" + name + "'" + negated + "'(" + writeParametersForOperator(operator) + ")'(" + writeOperatorReferenceForOperator(operator) + ")?"
						+	"\n;\n\n";
			}
		}
		return code;
	...
	private static void writeFile(String code){
		String filepath = (ResourcesPlugin.getWorkspace().getRoot().getLocation().toFile().toString() + "/../MQLApplication/org.MQLApplication/src/org/xtext/MQL.xtext").replace("\\", "/");
