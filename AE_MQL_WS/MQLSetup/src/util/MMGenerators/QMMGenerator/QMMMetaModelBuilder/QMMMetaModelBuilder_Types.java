...
public class QMMMetaModelBuilder_Types {
	...
	static EClassifier getJavaTypeForEType(EClassifier ed){
		String oldTypeName = ed.getName();
		String newTypeName;
		switch (oldTypeName) {
			case "EString":
				newTypeName = "String";
		...
		if(newTypeName!=null){
			if(!QMMMetaModelBuilder_Collections.eClassifiersMap.containsKey(newTypeName)){
				EClassifier newType = EObjectHelper.createEDataTypeForName(newTypeName);
				QMMMetaModelBuilder_Collections.eClassifiersMap.put(newTypeName, newType);
				return newType;
			}else{
				return QMMMetaModelBuilder_Collections.eClassifiersMap.get(newTypeName);
			}
		}else{
			return null;
		}
