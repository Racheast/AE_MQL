...
public class EClassMethodBodies {
	...
	public static final String CONCRETE_REFERENCE_VALIDATIONOPERATION_BODY = "super.validate(diagnostic, context);"
			+	"\nif(this.orGroup != null && this.orGroup instanceof qmm.Reference_OrGroup){"
			+	"\n		EList<qmm.OrGroup_Type> references = (EList<qmm.OrGroup_Type>)this.orGroup.eGet(this.orGroup.eClass().getEStructuralFeature(\"reference\"));"
			+	"\n"	
			+	"\n		java.util.ArrayList<org.eclipse.emf.ecore.EObject> thisContainers = new java.util.ArrayList<org.eclipse.emf.ecore.EObject>();"
			+	"\n		org.eclipse.emf.ecore.EObject thisContainer = this.eContainer();"
			+	"\n"
			...
			+	"\n		}"
			+	"\n"
			+	"\n		for(qmm.OrGroup_Type ogt: references){"
			+ 	"\n			org.eclipse.emf.ecore.EObject eContainer = ogt.eContainer();"
			+ 	"\n			while(eContainer != null && !thisContainers.contains(ogt)){"
			+	"\n				if(eContainer instanceof qmm.Containment_OrGroup){"
			+ 	"\n					EList containment = (EList)eContainer.eGet(eContainer.eClass().getEStructuralFeature(\"containment\"));"
			...
			+ 	"\n"
			+	"\n					if(containment.size() > 1){"
			+	"\n						String errorMessage = ((qmm.OrGroup)ogt).getName() + \" is defined within an alternative Or-branch of node \" + ((qmm.OrGroup)eContainer).getName() + \". It's not possible to reference nodes that are defined in alternative Or-branches.\";"
			+	"\n						diagnostic.add(new org.eclipse.emf.common.util.BasicDiagnostic(org.eclipse.emf.common.util.Diagnostic.ERROR, qmm.util.QmmValidator.DIAGNOSTIC_SOURCE,qmm.util.QmmValidator.REFERENCE_OBJECT__VALIDATE, errorMessage,new Object[] { this }));"
			+	"\n					}"
			+	"\n				}else if(eContainer instanceof qmm.ReferenceObject && ((qmm.ReferenceObject)eContainer).isNegated()){"
			+	"\n						if(!thisContainers.contains(eContainer)){"		
			+	"\n							String errorMessage = ((qmm.OrGroup)ogt).getName() + \" is a negated pattern or is defined within a negated pattern. It's not possible to reference nodes that are negated or are defined within negated patterns.\";"
			+	"\n							diagnostic.add(new org.eclipse.emf.common.util.BasicDiagnostic(org.eclipse.emf.common.util.Diagnostic.ERROR, qmm.util.QmmValidator.DIAGNOSTIC_SOURCE,qmm.util.QmmValidator.REFERENCE_OBJECT__VALIDATE, errorMessage,new Object[] { this }));"
			+	"\n						}"
